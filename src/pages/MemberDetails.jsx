...
export default function MemberDetails() {
  ...
  if (loading) return <div className="min-h-[60vh] flex justify-center items-center text-primary-600"><Loader2 className="animate-spin w-10 h-10" /></div>;

  if (!member) return (
    <div className="text-center py-20 bg-[var(--bg-card)] rounded-3xl border border-[var(--border-color)] shadow-sm">
        <div className="w-16 h-16 bg-slate-100 dark:bg-slate-800 rounded-full flex items-center justify-center mx-auto mb-4">
            <AlertCircle className="text-slate-400" size={32} />
        </div>
        <h2 className="text-xl font-bold text-[var(--text-main)]">Member not found</h2>
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 pb-20 animate-in fade-in duration-500">
      ...
      <div className="flex items-center gap-4">
        <button onClick={() => navigate(-1)} className="p-2 rounded-xl hover:bg-[var(--bg-card)] hover:shadow-sm text-[var(--text-muted)] hover:text-[var(--text-main)] transition-all border border-transparent hover:border-[var(--border-color)]">
            <ArrowLeft size={20} />
        </button>
        <h1 className="text-xl font-bold text-[var(--text-main)] hidden md:block">Member Profile</h1>
      </div>

      {/* 2. PROFILE BANNER */}
      <div className="relative bg-[var(--bg-card)] rounded-[2rem] border border-[var(--border-color)] overflow-hidden shadow-sm group">
         <div className="absolute top-0 w-full h-32 bg-gradient-to-r from-primary-600 to-indigo-800 transition-all group-hover:scale-105 duration-700" />
         
         <div className="relative px-8 pb-8 pt-20 flex flex-col md:flex-row gap-6 items-start md:items-end">
             {/* Avatar */}
             <div className="w-24 h-24 rounded-2xl bg-[var(--bg-card)] p-1.5 shadow-xl -mt-6">
             ...
             <div className="flex-1 min-w-0 pb-1">
                <div className="flex flex-wrap items-center gap-3 mb-1">
                    <h2 className="text-3xl font-bold text-[var(--text-main)] tracking-tight">{member.name}</h2>
                    <span className={clsx("inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-bold uppercase tracking-wider border", 
                        member.role === 'admin' 
                        ? "bg-primary-50 dark:bg-primary-900/20 text-primary-700 dark:text-primary-300 border-primary-100 dark:border-primary-900/30" 
             ...
             <div className="flex gap-3 w-full md:w-auto mt-4 md:mt-0">
                 {!isYearClosed && activeClub?.role === "admin" && (
                    <Button onClick={() => setShowChandaModal(true)} leftIcon={<Plus size={18} />} className="shadow-lg shadow-primary-200 dark:shadow-none w-full md:w-auto">
                        Add Fee
                    </Button>
                 )}
             </div>
      ...
      {/* 3. MAIN CONTENT */}
      {isYearClosed ? (
        <div className="bg-slate-50 dark:bg-slate-900 border border-[var(--border-color)] rounded-[2rem] p-10 flex flex-col items-center justify-center text-center min-h-[400px]">
           <div className="w-16 h-16 bg-[var(--bg-card)] rounded-full flex items-center justify-center mb-6 shadow-sm ring-4 ring-slate-100 dark:ring-slate-800">
              <Lock className="w-8 h-8 text-slate-400" />
           </div>
           <h3 className="text-2xl font-bold text-[var(--text-main)]">Financial Records Hidden</h3>
           <p className="text-[var(--text-muted)] max-w-md mt-3 leading-relaxed">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-6">
                ...
                <div className="flex justify-between items-start mb-4">
                    <div className="flex-1">
                        ...
                        >
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-bold text-[var(--text-main)]">Subscription Status</span>
                            
                            <span className="md:hidden flex items-center justify-center text-[var(--text-muted)] p-1 bg-slate-50 dark:bg-slate-800 rounded-md border border-[var(--border-color)] group-active:scale-95 transition-transform">
                    ...
                    <div className="text-right pl-4">
                        <span className="block text-xs font-bold text-[var(--text-muted)] uppercase tracking-wider">Paid</span>
                        <span className={clsx("text-2xl font-bold font-mono", progressPercent === 100 ? "text-emerald-600 dark:text-emerald-400" : "text-primary-600 dark:text-primary-400")}>
                            {progressPercent}%
                        </span>
                    </div>
                </div>
                ...
                <div className={clsx("transition-all duration-300 overflow-hidden", isExpanded ? "max-h-[2000px] opacity-100" : "max-h-0 opacity-0 md:max-h-none md:opacity-100")}>
                    ...
                    <div className="h-2.5 w-full bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden mb-6">
                        <div className="h-full bg-primary-600 dark:bg-primary-500 transition-all duration-1000 ease-out rounded-full" style={{ width: `${progressPercent}%` }} />
                    </div>
                    ...
                    <div className={clsx("grid gap-3", frequency === 'monthly' ? "grid-cols-3 sm:grid-cols-4 md:grid-cols-6" : "grid-cols-5 sm:grid-cols-8 md:grid-cols-10")}>
                        {subscription?.installments?.map((inst) => {
                            ...
                            <button
                                ...
                                inst.isPaid 
                                    ? "bg-emerald-500 border-emerald-500 text-white shadow-md shadow-emerald-200 dark:shadow-none" 
                                    : "bg-[var(--bg-card)] border-[var(--border-color)] text-[var(--text-muted)] hover:border-primary-300 dark:hover:border-primary-700 hover:text-primary-600 dark:hover:text-primary-400 hover:shadow-sm",
            ...
            <div className={clsx("space-y-6", frequency === 'none' && "lg:col-span-3")}>
                <div className="grid grid-cols-2 gap-4">
                    <Card noPadding className="bg-[var(--bg-card)] border-[var(--border-color)] p-4">
                        <p className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-wider mb-1">Total Paid</p>
                        <p className="text-xl font-bold text-emerald-600 dark:text-emerald-400 font-mono">₹{totalContribution.toLocaleString()}</p>
                    </Card>
                    {frequency !== 'none' && (
                        <Card noPadding className="bg-[var(--bg-card)] border-[var(--border-color)] p-4">
                            <p className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-wider mb-1">Current Due</p>
                            <p className={clsx("text-xl font-bold font-mono", stats.subDue > 0 ? "text-rose-500 dark:text-rose-400" : "text-[var(--text-muted)]")}>₹{stats.subDue.toLocaleString()}</p>
                        </Card>
                    )}
                </div>
                ...
                <Card className="min-h-[300px]">
                    ...
                    {chandaHistory.length === 0 ? (
                        <div className="text-center py-8 text-[var(--text-muted)] bg-slate-50/50 dark:bg-slate-800/50 rounded-xl border border-dashed border-[var(--border-color)]">
                        ...
                        <div className="space-y-0 relative">
                            ...
                            <div key={fee._id} className="relative pl-8 py-3 group">
                                <div className="absolute left-2 top-4 w-3.5 h-3.5 bg-[var(--bg-card)] border-2 border-rose-200 dark:border-rose-900 rounded-full group-hover:border-rose-500 transition-colors" />
                                <div className="flex justify-between items-center">
                                    <div>
                                        <p className="text-sm font-bold text-[var(--text-main)]">Festival Payment</p>
                                        <p className="text-[10px] text-[var(--text-muted)]">{new Date(fee.createdAt).toLocaleDateString()}</p>
                                    </div>
                                ...
                                {fee.notes && <p className="text-xs text-[var(--text-muted)] mt-1 italic">"{fee.notes}"</p>}
                            </div>
                            ))}
                        </div>
                    )}
                ...
                <div className="bg-slate-900 dark:bg-black rounded-2xl p-6 text-white shadow-lg border border-slate-800">
